...
public interface ApplicantAppointmentDetailsRepository extends JpaRepository<ApplicantAppointmentDetails, String> {
    ...
    @Query(value = "SELECT `aad`.`appointment_id`,`aad`.`appointment_type`,`ad`.`applicant_name`, " +
            "`ad`.`applicantnic`,`ad`.`applicant_tp`,`aad`.`appointment_details` " +
            "FROM `applicant_appointment_details` `aad`,`applicant_details` `ad` " +
            "WHERE `aad`.`applicant_email`=`ad`.`applicant_email`",nativeQuery = true)
